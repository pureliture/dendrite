...
- Hermes drain은 locator-only pointer 문서를 ship한다. SQLite body를 읽지 않는다.
  (실제 세션 본문 추출은 neurons로 위임, dendrite scope 밖.)
...
- 본 scope에서는 위 미결정이 설계를 막지 않는다. Phase 2 구현 중 실제 코드 제약이
  요구사항 변경을 요구하면 `grill-to-spec` Phase 1로 회귀한다.
